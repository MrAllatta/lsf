"""planning/models.py"""
  ...
  class PlanningYear(models.Model):
      ...
      status = models.CharField(
          max_length=20,
          choices=[('planning', 'Planning'), ('active', 'Active'),
                   ('complete', 'Complete'), ('archived', 'Archived')],
          default='planning'
      )
      overplant_factor = models.DecimalField(max_digits=4, decimal_places=2, 
  ...
  class Planting(models.Model):
      planning_year = models.ForeignKey(PlanningYear, on_delete=models.CASCADE,
                                        related_name='plantings')
      revision_of = models.ForeignKey('self', on_delete=models.SET_NULL,
      ...
      crop_season = models.ForeignKey('reference.CropBySeason', 
      ...
      actual_total_yield = models.DecimalField(max_digits=10, decimal_places=2,
                                               null=True, blank=True)
      
      status = models.CharField(max_length=20, choices=PlantingStatus.choices,
                                default=PlantingStatus.PLANNED)
      ...
      def save(self, *args, **kwargs):
          ...
          if not self.planned_first_harvest_date and self.planned_plant_date:
              self.planned_first_harvest_date = (
                  self.planned_plant_date + 
                  timedelta(days=self.crop_season.dtm_days)
              )
          if not self.planned_last_harvest_date and self.planned_first_harvest_date:
              self.planned_last_harvest_date = (
                  self.planned_first_harvest_date + 
                  timedelta(weeks=self.crop_season.harvest_weeks - 1)
              )
          if not self.planned_total_yield:
              self.planned_total_yield = (
                  self.planned_bedfeet * 
                  self.crop_season.total_yield_per_bedfoot
              )
      ...
      def generate_nursery_events(self):
          ...
          seed_date = self.planned_plant_date - timedelta(
              weeks=self.crop.nursery_weeks
          )
          ...
          if self.crop.weeks_until_pot_up:
              pot_up_date = seed_date + timedelta(
                  weeks=self.crop.weeks_until_pot_up
              )
              NurseryEvent.objects.create(
                  planting=self,
                  event_type='pot_up',
      ...
      def generate_harvest_events(self):
          ...
          weekly_yield = (
              self.crop_season.weekly_yield_per_bedfoot * 
              self.planned_bedfeet
          )
  ...
  class HarvestEvent(models.Model):
      ...
      actual_bins = models.DecimalField(max_digits=6, decimal_places=1,
                                        null=True, blank=True)
      actual_bin_type = models.CharField(max_length=50, blank=True)
      actual_hours = models.DecimalField(max_digits=5, decimal_places=1,
                                         null=True, blank=True)
